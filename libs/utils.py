...
def entropy_regularization(pred, beta_erl=0.0):
	reg_loss = pred*tf.math.log(pred+1e-5)
	reg_loss += (1.0-pred)*tf.math.log(1.0-pred+1e-5)
	reg_loss *= beta_erl
	return reg_loss


def get_loss_function(loss_type, label_smoothing=0.0):
    loss_fn = None
    if loss_type == 'bce':
